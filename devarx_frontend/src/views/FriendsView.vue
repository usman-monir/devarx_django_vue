        ...
        <div class="main-left col-span-3 space-y-4">
                        ...
                        <div v-for="request in friendRequests" :key="request.id">
                            <div class="p-3 flex flex-col justify-around items-center bg-gray-100 rounded-lg">
                                ...
                                <div class="mt-6 flex space-x-8 justify-around">
                                    <button @click.prevent="acceptRequest(request.send_by.id)"
                                        class="inline-block mt-4 p-2 bg-purple-600 text-white text-s rounded-md">Accept</button>
                                    <button @click.prevent="rejectRequest(request.send_by.id)"
                                        class="inline-block mt-4 p-2 bg-red-600 text-white text-s rounded-md">Reject</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <template v-if="$route.params.id != userStore.user.id">
                ...
                <div v-if="mutualFriends.length"
                    class="p-6 bg-white border border-gray-200 rounded-lg grid grid-cols-3 gap-4">
